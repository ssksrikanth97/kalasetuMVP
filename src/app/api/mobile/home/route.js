...
export async function GET(request) {
    try {
        ...
        const categoriesSnapshot = await adminDb.collection('categories').limit(10).get();
        const categories = categoriesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        // 3. Fetch "New Arrivals" (Assuming recent products)
        const newArrivalsSnapshot = await adminDb.collection('products')
        ...
        const discountedSnapshot = await adminDb.collection('products')
        ...
        if (discountedProducts.length === 0) {
            const fallbackDocs = await adminDb.collection('products').limit(5).get();
            discountedProducts = fallbackDocs.docs.map(doc => ({ ...doc.data(), id: doc.id, tag: '50% Off' }));
        }

        // 5. Fetch "Suggested For You"
        const suggestedSnapshot = await adminDb.collection('products')
